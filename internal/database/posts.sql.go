...
const getAllPostsFromFollowingFeeds = `-- name: GetAllPostsFromFollowingFeeds :many
...
func (q *Queries) GetAllPostsFromFollowingFeeds(ctx context.Context, userID uuid.UUID) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, getAllPostsFromFollowingFeeds, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
